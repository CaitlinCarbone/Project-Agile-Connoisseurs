...
const exportedMethods = {
  async createProject(
    ...
    projectDescription = validation.checkString(
      projectDescription,
      "Project Description"
    );
    ...
    projectStartDate = validation.checkString(
      projectStartDate,
      "Project Start Date"
    );
    ...
    if (!Array.isArray(requiredDocuments))
      throw "Required Documents must be an array";
    for (let i = 0; i < requiredDocuments.length; i++) {
      if (typeof requiredDocuments[i] !== "string")
        throw "Required Documents must be an array of strings";
    }
